...
import ru.mirea.feofanov.httpurlconnection.databinding.ActivityMainBinding;
...
public class MainActivity extends AppCompatActivity {
    ...
    public void onClickWeather(View view) {
        ...
        if (networkinfo != null && networkinfo.isConnected()) {
            new DownloadPageTaskWeather().execute("https://api.openweathermap.org/data/2.5/weather?lat="+latitude+"&lon="+longitude+"&appid=f5142a88db70994cdb0f2e5169cc00bf"); // запуск нового потока
        } else {
            Toast.makeText(this, "Нет интернета", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private class DownloadPageTaskWeather extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... urls) {
            try {
                return downloadIpInfoWeather(urls[0]);
            } catch (IOException e) {
                e.printStackTrace();
                return "error";
            }
    ...
    private String downloadIpInfoWeather(String address) throws IOException {
        InputStream inputStream = null;
        String data = "";
        try {
            URL url = new URL(address);
